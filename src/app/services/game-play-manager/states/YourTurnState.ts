...
export class YourTurnState extends TurnState {
	...
	public claimSegment(
		...
		gamePlayManagerService.serverProxy
			.claimSegment(segment, c)
	...
	public openClaimSegmentModal(gamePlayManagerService: GamePlayManagerService, s: Segment): void {
		...
		if (segmentColor === BusColor.Rainbow) {
			...
			if (!canClaimGray(s.length, cardCounts)) {
				...
				gamePlayManagerService.toastError(`You don't have enough cards for that!`);
				return;
			}
		} else {
			// Colored segment
			const playerColorCount = playerCards.filter(
				card => card.color === segmentColor || card.color === BusColor.Rainbow
			).length;
