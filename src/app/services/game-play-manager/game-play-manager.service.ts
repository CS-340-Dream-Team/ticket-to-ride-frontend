...
import TurnState, { GameInitState, GameOverState, NotYourTurnState, YourTurnState } from "./states";
...
export class GamePlayManagerService {
	...
	poll(serverProxy: ServerProxyService) {
		...
		serverProxy
			.getGameData(this.lastCommandId)
			.then(commands => {
						...
						if (command.player === this.clientPlayer.name) {
							...
							this._allPlayers.forEach((player, index) => {
								if (player.name === command.player) {
									...
									this._routeDeckSizeSubject.next(this._routeDeckSize);
									this._allPlayers[index].routeCards = (this._allPlayers[index]
										.routeCards as Route[]).concat(command.privateData["cardsKept"]);
				...
				case "drawTen":
					if (this.updateLastCommandID(command.id)) {
						this._allPlayers.forEach((player, index) => {
							if (player.name === command.player) {
								if (player.name === this.clientPlayer.name) {
									(this._allPlayers[index].busCards as BusCard[]).push(
										...command.privateData["cards"]
									);
								} else {
