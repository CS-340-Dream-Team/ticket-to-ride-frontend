...
export class ClaimSegmentComponent {
	...
	public display(s: Segment, p: Player) {
		...
		if (s) {
			this.selectedColor =
				this.segment && this.segment.color === BusColor.Rainbow ? BusColor.Red : this.segment.color;
		}
	}
	...
	get colorsPlayerHas(): string[] {
		return Object.keys(getPlayerCardCounts(<BusCard[]>this._player.busCards))
			.filter(
				color =>
					getPlayerCardCounts(<BusCard[]>this._player.busCards)[color] !== 0 &&
	...
	get numberOfColorInHand(): number {
		return getPlayerCardCounts(<BusCard[]>this._player.busCards)[
			this._busColorToString(this.selectedColor).toLowerCase()
		];
	}
